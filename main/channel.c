...
static void send_connected_event(struct ast_channel *chan, const char *event_id);
...
void ast_deactivate_generator(struct ast_channel *chan)
{
	//LeaseHawk Addition - JZ
	if (option_debug)
    	ast_log(LOG_DEBUG, "In ast_deactivate_generator with channel %s\n", chan->name);
	ast_channel_lock(chan);
		...
		*/
		...
		case AST_FRAME_CONTROL:
			if (f->subclass == AST_CONTROL_ANSWER) {
				...
				//LeaseHawk Addition to send Connected AMI Event - JZ.
				send_connected_event(chan, "Connected");
			}
			break;
		case AST_FRAME_DTMF_END:
