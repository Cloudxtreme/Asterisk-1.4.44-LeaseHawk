...
//LeaseHawk addition for send_dial_progress_event function - JZ.
static void send_dial_progress_event(struct ast_channel *chan, const char *event_id, char *userdata);
...
static struct ast_channel *wait_for_answer(struct ast_channel *in, struct dial_localuser *outgoing, int *to, struct ast_flags *peerflags, int *sentringing, char *status, size_t statussize, int busystart, int nochanstart, int congestionstart, int priority_jump, int *result, char *userdata, int useEarlyMedia)
{
	...
	//LeaseHawk Added connectedsignal
	char connectedsignal[10];
	...
	if (single) {
		...
		//LeaseHawk addition to send "Trying" event over AMI - JZ
		if (option_debug)
                ast_log(LOG_DEBUG, "Sending trying manager event for channel %s with data %s\n", outgoing->chan->name, userdata);
        send_dial_progress_event(outgoing->chan, "Trying", userdata);
	}

	//LeaseHawk Addition
    if (option_debug)
		ast_log(LOG_DEBUG, "Timeout value at start for %s, user data %s = %d\n", outgoing->chan->name, userdata, *to);
	...
	while (*to && !peer) {
		...
		for (o = outgoing; o; o = o->next) {
			...
			if (f->frametype == AST_FRAME_CONTROL) {
				switch(f->subclass) {
					//LeaseHawk Change to allow Progressing Manager Event. - JZ
				case AST_CONTROL_PROGRESS:
					...
					if (!ast_test_flag(outgoing, OPT_RINGBACK)) {
			                ...
			                if (option_debug)
                       			ast_log(LOG_DEBUG, "Sending progressing manager event for channel %s with data %s\n", c->name, userdata);
			                send_dial_progress_event(c, "Progressing", userdata);
                            /* If we have early media, fall through to answer */
                            if(!useEarlyMedia)
                            	break;
					}
					//LeaseHawk Change: If there is Early Media, then fall through to Answer so no break here. - JZ
					//break;				
				case AST_CONTROL_ANSWER:
					...
					// Set channel variable to discriminate between progressing and connect
					if(f->subclass == AST_CONTROL_PROGRESS)
						strcpy(connectedsignal, "PROGRESS");
					else if(f->subclass == AST_CONTROL_ANSWER)
						strcpy(connectedsignal, "ANSWER");
			                if (option_debug)
                       				ast_log(LOG_DEBUG, "Setting channel variable CONNECTED_SIGNAL to %s\n", connectedsignal);
					pbx_builtin_setvar_helper(c, "CONNECTED_SIGNAL", connectedsignal);
...
static int dial_exec_full(struct ast_channel *chan, void *data, struct ast_flags *peerflags, int *continue_exec)
{
	...
	AST_DECLARE_APP_ARGS(args,
			     ...
			     AST_APP_ARG(timeout);
			     AST_APP_ARG(options);
			     AST_APP_ARG(userdata);
			     AST_APP_ARG(earlymediaflag);
			     AST_APP_ARG(url);
	);
	...
	//LeaseHawk Addition --JZ.
	if (option_debug)
        ast_log(LOG_DEBUG, "Dialing with user data %s\n", args.userdata);
    if (option_debug)
        ast_log(LOG_DEBUG, "Dialing with early media flag %s\n", args.earlymediaflag);

    useEarlyMedia = ((args.earlymediaflag!=NULL)?1:0);
	...
	time(&start_time);
	//LeaseHawk Change to allow early media dial. - JZ
	//peer = wait_for_answer(chan, outgoing, &to, peerflags, &sentringing, status, sizeof(status), numbusy, numnochan, numcongestion, ast_test_flag(&opts, OPT_PRIORITY_JUMP), &result);
	peer = wait_for_answer(chan, outgoing, &to, peerflags, &sentringing, status, sizeof(status), numbusy, numnochan, numcongestion, ast_test_flag(&opts, OPT_PRIORITY_JUMP), &result, args.userdata, useEarlyMedia);
...
//LeaseHawk Addition for call progress AMI events. - JZ
static void send_dial_progress_event(struct ast_channel *chan, const char *event_id, char *userdata)
{
        if (option_debug)
                ast_log(LOG_DEBUG, "Sending event %s\n", event_id);
        manager_event(EVENT_FLAG_CALL, event_id,  
                           "ChannelName: %s\r\n"
                           "ChannelID: %s\r\n"
                           "UserData: %s\r\n",
                           chan->name, 
